...
export default function reducer(state, { type, payload }) {
  ...
  const makeLogicOperations = (operator) => {
    ...
    if (state.operation === "/" && state.currentNumber === "0")
      return { ...initialState, currentNumber: "Cannot divide by zero" };

    return {
      currentNumber: "0",
      previousNumber: `${makeMathOperations(state.operation)}`,
      operation: operator,
    };
  };
  ...
  switch (type) {
      ...
      return {
        ...
        currentNumber:
          state.currentNumber === "0"
            ? payload.digit
            : `${state.currentNumber}${payload.digit}`,
      };
    ...
    case ACTIONS_TYPES.delete:
      if (
        state.operation === "=" ||
        state.currentNumber === "Cannot divide by zero"
      )
        return initialState;
    ...
    case ACTIONS_TYPES.equalOperation:
      ...
      return {
        currentNumber: `${makeMathOperations(state.operation)}`,
    ...
    case ACTIONS_TYPES.invertSign:
      ...
      if (state.currentNumber.includes("-"))
        return { ...state, currentNumber: state.currentNumber.slice(1) };
